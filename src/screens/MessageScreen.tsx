...
export function MessageScreen({ onNext }: { onNext: () => void }) {
  ...
  return (
    <Screen bare>
      ...
      {/*
        Ведущий стоит на нижнем крае кадра и виден целиком — от каски до пояса,
        с рукой и колбой. Ради этого подвинут ТЕКСТ, а не он: реплика ушла
        наверх, в небо над городом, и больше не ложится ему на лицо и на жест.
        Раньше пробовали наоборот — поднимали фигуру над текстовым слоем и
        растворяли ей низ маской; получалось, что ведущий висит посреди экрана
        и всё равно обрезан.

        Высота — минимум из трёх ограничений, и каждое настоящее:
        • var(--tg-vh) - 170px — сверху нужна полоса под реплику, иначе на
          коротком экране текст ляжет ведущему на каску;
        • 140vw — рука с указующим жестом доходит до самого края исходника
          (соотношение сторон 0.711), и выше этого её срезало бы боковой
          границей экрана. Поэтому же выключен bleed;
        • app-max × 1.4 — то же ограничение по ширине на широком окне, где
          приложение стоит колонкой и vw уже не про него.

        На фазе idle персонажа нет вовсе: карточка сообщения остаётся
        единственным, что тянет внимание (DESIGN.md §8.1).
      */}
      {phase !== 'idle' && (
        <Character
          height="min(calc(var(--tg-vh) - 170px), 140vw, calc(var(--app-max) * 1.4))"
          bleed={false}
          delay={0.1}
        />
      )}
      ...
      <div className="relative z-20 flex flex-1 flex-col px-[var(--gutter)] pt-sp5">
        ...
        {/* Реплика сцены — в небе над городом, НАД ведущим. Внизу для неё места
            нет: там стоит он сам, и любой текст ложится ему на лицо или на
            жест. Читаемость держат затемнение кадра сверху и подложка блока. */}
        {phase === 'voice' &&
          (voice.line?.accent ? (
            <CityAccent text={voice.line.accent} className="mt-sp4" />
          ) : (
            <Subtitles line={voice.line} className="mt-sp4" />
          ))}
