...
export function MessageScreen({ onNext }: { onNext: () => void }) {
  ...
  return (
    <Screen bare>
      ...
      {/*
        Фигура стоит НАД текстовым слоем, а не на нижнем крае экрана: иначе
        субтитры и панель голосового закрывают ей весь торс и от рассказчика
        остаётся голова с плечом. Высота подобрана так, чтобы человек целиком
        помещался между меткой района сверху и репликой снизу.
      */}
      {phase !== 'idle' && <Character height="46vh" bottom="40%" delay={0.1} />}
